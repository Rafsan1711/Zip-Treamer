...
Collecting chess<2,>=1 (from python-chess)
  Downloading chess-1.11.2.tar.gz (6.1 MB)
     ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ 6.1/6.1 MB 50.8 MB/s eta 0:00:00
...
if os.path.exists(raw_file_path):
    ...
    print("⏳ Downloading database (1.5 GB)... This may take 5-8 minutes.")
...
# --- কনফিগারেশন ---
# প্রতিবার রান করলে কতগুলো গেম প্রসেস করবে
GAMES_PER_RUN = 500000
# টার্গেট পজিশন (৫০ লক্ষ)
TARGET_POSITIONS = 5000000
MIN_ELO = 2000

# পাথ সেটআপ (Drive & Local)
...
def save_checkpoint(games_proc, unique_pos):
    ...
    with open(drive_checkpoint_path, 'w') as f:
        json.dump(data, f)
    print(f"📌 Checkpoint Saved: {games_proc:,} games processed.")
...
def process_batch():
    ...
    print(f"🔄 Resuming from Game #{start_game_index:,}")
    print(f"📊 Current Unique Positions: {current_unique_positions:,}")

    # ২. ড্রাইভ থেকে ডেটাবেস কপি করে লোকালে আনা (ফাস্ট স্পিডের জন্য)
    if os.path.exists(drive_db_path):
        print("📥 Copying Database from Drive to Colab (Faster I/O)...")
    ...
    ''')
    cursor.execute('PRAGMA synchronous = OFF') # স্পিড বুস্ট
    cursor.execute('PRAGMA journal_mode = MEMORY')

    # ৪. ফাইল রিডিং এবং স্কিপিং
    ...
    with open(raw_file_path, 'rb') as ifh:
        with dctx.stream_reader(ifh) as reader:
            ...
            # ফাস্ট স্কিপিং (Fast Forward)
            if start_game_index > 0:
                print(f"⏩ Skipping first {start_game_index:,} games... (Please wait)")
                skipped = 0
                while skipped < start_game_index:
                    # read_headers দ্রুত স্কিপ করতে পারে
                    if chess.pgn.read_headers(text_stream) is None:
                        print("❌ End of file reached during skipping!")
                        return
                    skipped += 1
                    if skipped % 50000 == 0: print(f"   Skipped {skipped:,} games...")
                print("✅ Skipping complete. Processing new games...")
                ...
                try:
                    ...
                    if white_elo >= MIN_ELO and black_elo >= MIN_ELO:
                        ...
                        for i, move in enumerate(game.mainline_moves()):
                            ...
                            # আমরা সরাসরি ব্যাচে যোগ করব, পরে DB তে মার্জ করব
                            # (সহজ করার জন্য আমরা এখানে সরাসরি SQL Upsert লজিক ব্যবহার করব না,
                            # বরং Python এ প্রসেস করে ব্যাচ ইনসার্ট করব)

                            # নোট: এখানে জটিলতা কমানোর জন্য সরাসরি DB তে কুয়েরি করছি না।
                            # আমরা ধরে নিচ্ছি ডুপ্লিকেট হলে SQLite হ্যান্ডেল করবে (INSERT OR IGNORE)
                            # কিন্তু সঠিক স্ট্যাটস আপডেটের জন্য আমাদের রিড করতে হবে।
                            # স্পিডের জন্য আমরা এই ব্যাচে শুধু নতুন পজিশনগুলো বাফারে রাখব।

                            # (এই অংশের লজিক একটু জটিল, তাই সহজ 'RAM Cache' ব্যবহার করছি)
                            # সিম্পলিসিটির জন্য: আমরা প্রতি গেমের পজিশনগুলো ব্যাচে রাখছি

                            # *PRO TIP*: এখানে আমরা ব্যাচ ইনসার্ট না করে সরাসরি আপডেট করব
                            # কারণ ডুপ্লিকেট পজিশন চেক করা জরুরি।
    ...
    print("💾 Saving Database to Google Drive...")
...
```text
🔄 Resuming from Game #4,500,000
📊 Current Unique Positions: 2,275,527
📥 Copying Database from Drive to Colab (Faster I/O)...
⏩ Skipping first 4,500,000 games... (Please wait)
...
✅ Skipping complete. Processing new games...
Keep Alive Signal Sent...
...
📌 Checkpoint Saved: 5,000,000 games processed.
...
# --- কনফিগারেশন ---
# আপনার টোকেনটি এখানে দিন (অবশ্যই Write পারমিশন ওয়ালা)
...
# ফাইলের সঠিক পাথ (যেখানে আমরা সেভ করেছি)
base_path = '/content/drive/MyDrive/Chessmate_Project/v2'
...
print(f"⚖️ Size: {os.path.getsize(db_path) / (1024*1024):.2f} MB")
print("⏳ This may take a while depending on file size...")

try:
    # ১. রিপোজিটরি তৈরি (যদি না থাকে)
    api.create_repo(repo_id=REPO_ID, repo_type="dataset", exist_ok=True)

    # ২. ফাইল আপলোড (Direct Stream)
    # আমরা ফাইলের নাম পরিবর্তন করে 'chess_stats_v2.db' দিচ্ছি ভার্সন ট্র্যাকিংয়ের জন্য
    api.upload_file(
    ...
    print("\n✅ UPLOAD SUCCESSFUL!")
    print("🎉 Your massive dataset is now secured on Hugging Face.")
    print(f"🔗 URL: https://huggingface.co/datasets/{REPO_ID}/resolve/main/chess_stats_v2.db")
...
```text
🚀 Starting Upload to: Rafs-an09002/chessmate-data-v2
📂 File: /content/drive/MyDrive/Chessmate_Project/v2/chessmate_v2.db
⚖️ Size: 882.46 MB
...
New Data Upload               : 
  0.00B /  0.00B,  0.00B/s  
  ...roject/v2/chessmate_v2.db: 100%
  925MB /  925MB            
No files have been modified since last commit. Skipping to prevent empty commit.
WARNING:huggingface_hub.hf_api:No files have been modified since last commit. Skipping to prevent empty commit.

✅ UPLOAD SUCCESSFUL!
🎉 Your massive dataset is now secured on Hugging Face.
🔗 URL: https://huggingface.co/datasets/Rafs-an09002/chessmate-data-v2/resolve/main/chess_stats_v2.db
```
